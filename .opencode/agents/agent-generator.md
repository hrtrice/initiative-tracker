...
description: Explores a project's codebase to understand its architecture, patterns, and workflows, then generates tailored project-level agents in .opencode/agents/. Use when setting up a new project or when the project needs specialized agents.
model: opus
mode: subagent
permissions:
  read: true
  write: true
  execute: ["ls", "tree", "find", "cat", "mkdir", "echo"]
output-contract: |
...
3. **Generate** high-quality agents in `.opencode/agents/`
...
ls -la .opencode/agents/ 2>/dev/null || echo "No agents directory yet"
ls -la rules/ 2>/dev/null || echo "No rules directory"
ls -la .opencode/skills/ 2>/dev/null || echo "No skills directory"
ls -la .opencode/skills/ 2>/dev/null || echo "No skills directory"
...
1. Create the `.opencode/agents/` directory if needed:
```bash
...
Create `AGENTS.md` — a quick-reference for the orchestrator:
...
2. Check for prior agent-generation runs -- look for existing `.opencode/agents/` files and `AGENTS.md` to avoid overwriting intentional customizations
