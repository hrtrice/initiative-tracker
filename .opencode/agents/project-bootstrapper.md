...
description: Bootstraps a new project with AGENTS.md, hooks, and OpenCode settings. Use when starting a new project or adding OpenCode support to an existing project.
model: opus
mode: primary
permissions:
  read: true
  write: true
  execute: ["ls", "cat", "which", "git log", "mkdir", "echo"]
output-contract: |
...
1. **AGENTS.md** — Project context Claude reads every session
...
```bash
# Check current state
ls -la .opencode/ 2>/dev/null || echo "No .claude directory"
cat AGENTS.md 2>/dev/null | head -20 || echo "No AGENTS.md"
...
Create a AGENTS.md that follows best practices. For each line, ask: "Would removing this cause Claude to make mistakes?" If not, cut it.
...
**Keep it under 200 lines.** Long AGENTS.md files cause instruction loss.

## Phase 3: Configure Hooks

Create `opencode.json` for team-shared settings:
    ...
    "PreCompact": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "mkdir -p .opencode/memory/sessions && echo '# Pre-Compaction State\n\nAuto-persisted: '$(date -Iseconds) > .opencode/memory/sessions/pre-compact.md"
          }
        ]
      }
    ]
...
Use hooks for things that MUST happen 100% of the time. Use AGENTS.md instructions for things that SHOULD usually happen.
...
Create `.opencode/settings.local.json` (gitignored) for personal settings:
...
```bash
mkdir -p .opencode/memory/sessions
mkdir -p .opencode/memory/agents
mkdir -p .opencode/memory/scratch
```

Create initial `MEMORY.md` at `.opencode/memory/MEMORY.md`:
...
- **Prefer writing files as you go.** Write `AGENTS.md` as soon as Phase 3 is complete rather than holding its content in memory through Phases 4-10.
...
2. Check whether this project has been bootstrapped before -- look for `.opencode/` and `AGENTS.md` to determine if this is a fresh setup or an update
...
- [ ] `opencode.json` exists with hooks + permissions
- [ ] `.opencode/settings.local.json` template exists
- [ ] `rules/` exists with at least `commits.md` and `definition-of-done.md`
- [ ] `.opencode/skills/` contains installed skills (blossom at minimum, plus review, retro, status, handoff for full workflow)
- [ ] `.opencode/memory/` directory created with initial MEMORY.md
- [ ] `.gitignore` updated
...
- Keep AGENTS.md under 200 lines — brevity is critical
