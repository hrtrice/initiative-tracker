...
- All file paths that would reference `.opencode/memory/agents/<name>/` now use `.opencode/memory/agents/solo/` instead
...
Read `.opencode/memory/agents/<name>/learnings.md` (team mode) or `.opencode/memory/agents/solo/learnings.md` (solo mode) if it exists -- note current line count as the baseline.

### 0c. Load Prior Capability Data

Check for existing capability tracking:
- Team mode: Read `.opencode/memory/agents/<name>/capability.yaml` if it exists
- Solo mode: Read `.opencode/memory/agents/solo/capability.yaml` if it exists
...
```bash
# Edit timeline
git log --oneline --follow -- .opencode/memory/agents/<name>/learnings.md

# Actual diffs to see what changed
git log -p --follow -- .opencode/memory/agents/<name>/learnings.md
```
...
```bash
mkdir -p .opencode/memory/agents/<name>/challenges   # team mode
mkdir -p .opencode/memory/agents/solo/challenges     # solo mode
```

Write challenges to `.opencode/memory/agents/<name>/challenges/<timestamp>-round-<N>-challenges.md` (team mode) or `.opencode/memory/agents/solo/challenges/<timestamp>-round-<N>-challenges.md` (solo mode):
...
- Learnings path: `.opencode/memory/agents/solo/learnings.md`
...
Write to `.opencode/memory/agents/<name>/challenges/<timestamp>-round-<N>-evaluation.md` (team mode) or `.opencode/memory/agents/solo/challenges/<timestamp>-round-<N>-evaluation.md` (solo mode):
...
Read `.opencode/memory/agents/<name>/learnings.md` (team mode) or `.opencode/memory/agents/solo/learnings.md` (solo mode). Append new entries tagged with challenge provenance:
...
Write or update `.opencode/memory/agents/<name>/capability.yaml` (team mode) or `.opencode/memory/agents/solo/capability.yaml` (solo mode):
...
```markdown
<!-- Solo mode only: -->
> **SOLO MODE** -- No team.yaml found. Ran as 'main-session'. Results written to .opencode/memory/agents/solo/.
...
- Team mode: `.opencode/memory/agents/<name>/capability.yaml`
- Solo mode: `.opencode/memory/agents/solo/capability.yaml`

---

## Guidelines

1. **Compaction resilience**: Per `rules/memory-layout.md`, checkpoint at phase boundaries to `.opencode/memory/scratch/active-learn-checkpoint.md`.
