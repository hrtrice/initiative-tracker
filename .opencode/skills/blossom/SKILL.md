...
12. **Persist epic state.** After reporting, write `.opencode/memory/epics/<epic-id>/epic.md` so downstream skills (/sprint) can load context across sessions.
...
14. **Compaction resilience**: Per `rules/memory-layout.md`, checkpoint at phase boundaries to `.opencode/memory/scratch/blossom-checkpoint.md`.

## Phase 7: Persist Epic State

After the Phase 6 report, write epic state to `.opencode/memory/epics/<epic-id>/epic.md` so that `/sprint` and other downstream skills can load blossom findings in a later session without re-exploring.

**Create the directory and file:**

```bash
mkdir -p .opencode/memory/epics/<epic-id>
```

Write `.opencode/memory/epics/<epic-id>/epic.md` with this structure:
