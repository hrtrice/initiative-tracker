...
- **challenge files** — One or more challenge files in `.opencode/memory/agents/<agent>/challenges/*-challenges.md`, generated by `/challenge-gen` ([how to generate](/challenge-gen))
    ...
    -> For each challenge, dispatch agent with scenario + criteria (NOT trap/truth)
      -> Evaluate agent output against hidden trap + ground truth
        -> Score: result, trap detection, confidence calibration
          -> Write evaluation results to .opencode/memory/agents/<name>/challenges/
...
git ls-files -- ".opencode/memory/agents/$ARGUMENTS/challenges/*-challenges.md"
```

Also check untracked files with Glob: `.opencode/memory/agents/<name>/challenges/*-challenges.md`
...
3. Read `.opencode/memory/agents/<name>/learnings.md` if it exists
...
Write evaluation results to `.opencode/memory/agents/<name>/challenges/<timestamp>-evaluation.md`:
