        ...
        -> Detect gaps (upcoming work domains vs. coverage)
          ...
          -> [rules] Compute passive context budget
            -> Emit pipe-format report
              ...
              -> [rules] Write review manifest to .opencode/memory/scratch/
```
...
1. Run `ls .opencode/memory/agents/` to list available agents
2. Ask: "Which agent should I curate? Available: [names]"
3. Stop and wait for response

If `$ARGUMENTS` is provided:
1. Check whether `.opencode/memory/agents/<name>/learnings.md` exists
2. If not found:
   ...
   - If the agent is NOT in team.yaml (or team.yaml does not exist): "No learnings file at .opencode/memory/agents/<name>/learnings.md. Has this agent been dispatched yet?" Stop and wait for response.
3. Note whether `.opencode/memory/agents/<name>/archive.md` exists (needed for Phase 4)
...
**Learnings mode (normal):** Read `.opencode/memory/agents/<name>/learnings.md`. Note:
...
```bash
ls .opencode/memory/epics/*/epic.md 2>/dev/null
```
...
```bash
ls .opencode/memory/agents/*/learnings.md 2>/dev/null
```
...
If `.opencode/memory/agents/<name>/archive.md` exists, read it. Scan for entries that match gap areas. These are **archive candidates** — entries that were previously archived but are now relevant again.
...
1. **KEEP: <entry title or first clause>** — <full entry text>
   ...
   - source: .opencode/memory/agents/<name>/learnings.md
...
3. **ADD (from archive): <entry title>** — <entry text>
   ...
   - source: .opencode/memory/agents/<name>/archive.md
...
4. **ADD (cross-agent): <entry title>** — <entry text>
   ...
   - source: .opencode/memory/agents/<other-name>/learnings.md
   - cross-agent: true
...
1. **SEED: <entry title>** — <entry text>
   ...
   - source: .opencode/memory/agents/<other-name>/learnings.md | .opencode/memory/agents/<other-name>/archive.md
   - cross-agent: true
   - related: <comma-separated list of related entry titles, if any>
...
Append each ARCHIVE entry to `.opencode/memory/agents/<name>/archive.md`:
...
> "Create .opencode/memory/agents/<name>/learnings.md with these seeded entries? (y/n)"

If the user approves:

**5a. Create learnings.md**

Create `.opencode/memory/agents/<name>/learnings.md` (and the directory if it does not exist) with:
...
Write a review manifest to `.opencode/memory/scratch/rules-review.md`:
