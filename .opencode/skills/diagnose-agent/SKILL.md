...
- **learnings file** — `.opencode/memory/agents/<agent>/learnings.md` must exist with at least 5 entries; below this threshold there is insufficient signal for meaningful weakness detection ([populated by /retro and /active-learn](/retro))
...
2. Check that `.opencode/memory/agents/<name>/learnings.md` exists
...
1. **Compaction resilience**: Per `rules/memory-layout.md`, checkpoint at phase boundaries to `.opencode/memory/scratch/diagnose-agent-checkpoint.md`.
