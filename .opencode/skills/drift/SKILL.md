...
1. **Compaction resilience**: Per `rules/memory-layout.md`, checkpoint at phase boundaries to `.opencode/memory/scratch/drift-checkpoint.md`. In parallel mode, write the merged inventory to the checkpoint before proceeding to Phase 2 — agent outputs are not recoverable after context loss.
