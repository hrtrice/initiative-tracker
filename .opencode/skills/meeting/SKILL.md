...
ls .opencode/memory/scratch/meeting-panelists.md 2>/dev/null
```

<!-- Condition: only if .opencode/memory/scratch/meeting-panelists.md exists -->
...
- *Start fresh* → Delete `.opencode/memory/scratch/meeting-panelists.md` and proceed to Phase 1.
...
After all panelists are spawned, record their agent IDs to `.opencode/memory/scratch/meeting-panelists.md` for compaction resilience and resume support:
...
```bash
rm -f .opencode/memory/scratch/meeting-panelists.md
```
