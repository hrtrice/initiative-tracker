...
ls .opencode/memory/agents/*/archive.md 2>/dev/null
```
...
Read each `.opencode/memory/agents/*/archive.md` that exists. Archive entries can reveal historical patterns — an entry archived and re-added across multiple cycles is a strong promotion signal.
...
```bash
git log --follow --oneline -- .opencode/memory/agents/<name>/learnings.md 2>/dev/null
```
...
```bash
git log --follow --oneline --since="21 days ago" -- .opencode/memory/agents/<name>/learnings.md 2>/dev/null
```
