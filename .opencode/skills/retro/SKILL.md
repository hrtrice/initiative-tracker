...
Read all `.opencode/memory/agents/*/learnings.md` files. For each member, assess:
...
2. **Archive stale entries** — Move entries older than 21 days (with no recent references) to `.opencode/memory/agents/<name>/archive.md`
...
Append a summary to `.opencode/memory/team/retro-history.md`:
