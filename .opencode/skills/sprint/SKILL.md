...
For each member, read `.opencode/memory/agents/<name>/learnings.md`. Note the current size (line count) and most recent entries.
...
3. **Append**: Add to `.opencode/memory/agents/<name>/learnings.md` with today's date and dispatch provenance
...
1. **Compaction resilience**: Per `rules/memory-layout.md`, checkpoint at phase boundaries to `.opencode/memory/scratch/sprint-checkpoint.md`.
