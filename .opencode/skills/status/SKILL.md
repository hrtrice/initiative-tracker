...
Read `.opencode/memory/sessions/last.md`. If it does not exist, note "No previous session data."
...
**If `.claude/team.yaml` exists**, read it to get the team name and member list. Then for each member, check `.opencode/memory/agents/<name>/learnings.md`:
...
### Last Session
[Contents of .opencode/memory/sessions/last.md — include verbatim, trimmed to first 20 lines if longer]
