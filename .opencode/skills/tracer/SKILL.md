...
After completing the report, extract architectural findings discovered during implementation and persist them to `.opencode/memory/project/domain.md`.
...
**Process**:
1. Read `.opencode/memory/project/domain.md` if it exists. If it does not, create it with the header:
   ```markdown
...
4. After writing, confirm in the report output: "Persisted N domain concepts to `.opencode/memory/project/domain.md`." (or "No new domain concepts to persist." if none).
...
1. **Compaction resilience**: Per `rules/memory-layout.md`, checkpoint at phase boundaries to `.opencode/memory/scratch/tracer-checkpoint.md`.
